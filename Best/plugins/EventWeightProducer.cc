#include "SKKU/Best/interface/EventWeightProducer.h"
...
{
  PileUpRD_ = pset.getParameter< std::vector<double> >("PileUpRD"),
  PileUpRDup_ = pset.getParameter< std::vector<double> >("PileUpRDup"),
  PileUpRDdn_ = pset.getParameter< std::vector<double> >("PileUpRDdn"),
  PileUpMC_ = pset.getParameter< std::vector<double> >("PileUpMC"),
...
void EventWeightProducer::produce(edm::Event& event, const edm::EventSetup& es)
{
  ...
  event.put(npileup, "npileup");
  event.put(weightin, "weightin");
  event.put(weight, "weight");
  event.put(weightplus, "weightplus");
  event.put(weightminus, "weightminus");
...
EventWeightProducer::beginJob(){
  ...
  unsigned int nbins = min(PileUpRD_.size(),PileUpMC_.size());
